...
export default function NavBar({ compact }) {
    ...
    const isCompact = typeof compact === 'boolean' ? compact : location.pathname === '/';
    ...
    if (isCompact) {
        return (
            <>
                ...
                {open && (
                    <div onClick={() => setOpen(false)} className="fixed inset-0 bg-black/40" aria-hidden
                    />
                )}
                ...
                <aside id="main-menu"
                    ...
                    <nav>
                        <ul className="flex flex-col gap-1">
                            {paginas.flatMap(pag =>
                                pag.children ? [pag, ...pag.children] : [pag]).map((pag) => (
                                    <li key={pag.to}>
                                        <NavLink to={pag.to}
            ...
            <div>
                <ul>
                    {paginas.map((pag) =>
                        !pag.children ? (
                            <li key={pag.to}>
                                <NavLink to={pag.to} className={({ isActive }) => linkClass(isActive)} >
                                    {pag.label}
                                </NavLink>
                            </li>
                        ) : (
                            <li key={pag.to}>
                                ...
                                <ul>
                                    {pag.children.map((child) => (
                                        <li key={child.to}>
                                            <NavLink to={child.to} className={({ isActive }) => linkClass(isActive)} >
                                                {child.label}
                                            </NavLink>
                                        </li>
                                    ))}
